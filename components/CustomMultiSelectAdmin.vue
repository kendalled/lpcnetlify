  ...
  <div class="space-y-1" @keydown.up="moveHighlightUp" @keydown.down="moveHighlightDown" @keydown.enter="selectHighlighted" @keydown.escape="escapeHandler">
    ...
    <div v-click-outside="clickAway" class="relative">
      <span class="inline-block w-full rounded-md shadow-sm">
        ...
        >
          <span v-if="selectedLength > 0" class="block truncate">
            {{ nameList }}
          </span>
...
export default {
  ...
  computed: {
    ...
    nameList () {
      const func = x => x.name
      return this.selected.map(func)
    },
    optionsLength () {
      return (this.options.length - 1)
    },
    focusID () {
      return ('listbox-items-' + this.options[this.highlighted].name)
    }
  },
  watch: {
